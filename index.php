...
$cur_players = array();
array_push($cur_players, "HecklingFext");
//array_push($cur_players, "DreddBaron");
...
for($i=0;$i<$cnt;$i++)
{
	if(preg_match("/<a class=\"fc-gtag-link\"/", $f[$i]))
	{
		$arr=explode(">", $f[$i]);
		$name=explode("<", $arr[1]);
		if (!in_array($name[0], $old_ps)) { array_push($cur_players, $name[0]); }
		//echo $name[0] . (in_array($name[0], $old_ps) ? "  --  FOUND" : "  --  NOT") . "<br />";
	}
}
